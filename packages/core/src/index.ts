...
export { AutonomyEngine, AutonomyPolicyCenter, DEFAULT_AUTONOMY_POLICY, type AutonomyHandler, type EnqueueAutonomyJob } from "./autonomy.js";
...
export class AtlasCore {
  ...
  status(): AtlasStatus { return { lifecycle: this.lifecycle, version: "2.0.0-alpha.1", startedAt: this.startedAt, modules: ["event-bus", "semantic-knowledge", "decision-v3", "guardian", "memory-v3", "agent-runtime", "market-intelligence", "content-studio", "distribution-center", "autonomy-engine", "offer-center", "learning-engine", "scale-engine", "company-orchestrator", "plugin-runtime", "permissions", "authentication", "durable-storage"].map((name) => ({ name, status: this.lifecycle === "running" ? "ready" : "stopped" })), ai: { provider: this.provider.name, model: this.provider.model, mode: this.provider.mode } }; }
