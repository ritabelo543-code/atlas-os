...
const readinessMessage: Record<string, string> = {
  "missing-official-access-token": "Falta concluir a autorização oficial desta conta.",
  "missing-public-https-url": "Falta uma URL pública HTTPS para a mídia.",
  "missing-url-ownership-verification": "Falta comprovar no TikTok a propriedade da URL usada pelas imagens.",
  "configured-unverified": "Conector configurado; a validação final ocorrerá antes da publicação.",
};
...
export default function DistributionPage() {
  ...
  const selectedReadiness = selected?.channel === "instagram" ? readiness?.integrations.instagramPublishing : selected?.channel === "tiktok" ? readiness?.integrations.tiktokPublishing : undefined;
  const liveReady = selectedReadiness?.ready ?? false;
